...
NODES = {
     0: "Centro",        1: "Batel",          2: "Agua Verde",
     3: "Bigorrilho",    4: "Portao",          5: "CIC",
     6: "Pinheirinho",   7: "Sitio Cercado",   8: "Boqueirao",
     9: "Cajuru",       10: "Bacacheri",       11: "Boa Vista",
    12: "Sao Francisco", 13: "Reboucas",       14: "Pilarzinho",
}
...
POS = {
     ...
     6: (1.60, 2.71),   7: (3.62, 3.26),   8: (4.35, 4.76),
     9: (4.61, 6.35),  10: (4.60, 7.71),  11: (4.07, 8.11),
    12: (3.30, 7.09),  13: (3.63, 6.39),  14: (2.92, 8.03),
}
...
def desenhar(ax, G, origem, destino, rota1=None, rota2=None, titulo=''):
    ...
    edge_labels = {(u, v): f"{d['base']}|{d['weight']}" for u, v, d in G.edges(data=True)}
    nx.draw_networkx_edge_labels(G, POS, ax=ax, edge_labels=edge_labels, font_size=6,
        bbox=dict(boxstyle='round,pad=0.1', fc='white', alpha=0.6))

    if rota2:
        nx.draw_networkx_edges(G, POS, ax=ax, edgelist=list(zip(rota2[:-1], rota2[1:])),
            edge_color='#e67e22', width=4.0, alpha=0.8, style='dashed')
    if rota1:
        nx.draw_networkx_edges(G, POS, ax=ax, edgelist=list(zip(rota1[:-1], rota1[1:])),
            edge_color='#27ae60', width=4.5, alpha=0.95)
...
def main():
    ...
    if input("Usar horario atual? [Enter = sim / 'm' = manual]: ").strip().lower() == 'm':
        hora = int(input("  Hora (0-23): "))
        dia  = int(input("  Dia (0=Segunda ... 6=Domingo): "))

    print("  Clima: (1) Normal  (2) Chuva  (3) Tempestade")
    clima = {'2': 'chuva', '3': 'tempestade'}.get(input("  Opcao [1]: ").strip(), 'normal')
    ...
    try:
        ...
        tempo_base2 = round(sum(G[u][v]['base'] for u, v in zip(rota2[:-1], rota2[1:])), 1)
